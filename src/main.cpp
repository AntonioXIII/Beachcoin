...
uint256 hashGenesisBlock("0x000003f707708775d9c91531d718167ae1d1bf6a589178f69606a0c16f8085f6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Beachcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Beachcoin Signed Message:\n";
...
{
    // Beachcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Beachcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "Beachcoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1490723131;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1194283;

        if (fTestNet)
        {
            block.nTime    = 1394023194;
            block.nNonce   = 1280927160;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xb0, 0xde }; // Beachcoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeachcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeachcoinMiner : ProcessBlock, block not accepted");
    }
